...
TEST_F(ReachabilityTest, ReachabilityConstructorError) {
   EXPECT_THROW({ ClassProject::Reachability  Reachability(0); }, std::runtime_error); // State size 0 should throw an error
}
...
TEST_F(ReachabilityTest, SetInitState) {
    ...
    EXPECT_THROW({ fsm2->setInitState({true, false, false}); }, std::runtime_error); // Too many states
...
TEST_F(ReachabilityTest, isReachableError) {
    // Test for state vectors with incorrect sizes
    EXPECT_THROW({ fsm2->isReachable({true, false, false}); }, std::runtime_error); // Too many states
...
TEST_F(ReachabilityTest, StateDistanceError) {
    ...
    EXPECT_THROW({ fsm2->stateDistance({true, false, false}); }, std::runtime_error); // Too many states
...
TEST_F(ReachabilityTest, SetTransitionFunctionsTest) {
    ...
    EXPECT_THROW(fsm2->setTransitionFunctions(invalidTransitionFunctions), std::runtime_error); // Too few functions

    invalidTransitionFunctions.push_back(fsm2->neg(s1));
    invalidTransitionFunctions.push_back(s0); // Extra function
    EXPECT_THROW(fsm2->setTransitionFunctions(invalidTransitionFunctions), std::runtime_error); // Too many functions
    ...
    EXPECT_THROW(fsm2->setTransitionFunctions(invalidTransitionFunctions), std::runtime_error); // Unknown ID
}
#endif
